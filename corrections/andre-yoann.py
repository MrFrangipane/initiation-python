...
#_______09.2017
#_____Yoann André IIM A5 3D 
#______ EXO PYTHON - 3DSMAX 2016 
import math
import MaxPlus
import random
#------------- 3DS MAX -----------
def cylinder(radius,height): #création d'un cylinder
...
def move(node, position): #Positionné le cylinder
...
def compute_random_heights(count,height_min, height_max): 
	...
	for i in range(0,count):
	  nb = random.randint(height_min,height_max)
	  random_heights.append(nb)

	return random_heights #renvoie d'une liste avec des valeurs random
	
def padding(count,radius): #pas entre les cylinders
	padding_list = list()
	if radius <= 1:
	  radius += 1
	for i in range(0,count):
	...
	return padding_list #revoie du "pas" entre les objets
...
def compute_position(count,radius):
	...
	return position #renvoie d'une liste de position
...
#------------- CHEF -----------
def cylinder_square (cylinder_radius, cylinder_height, square_count, base_color1, base_color2): # CHEF 
	cylinders = list()
	radius_list = padding(square_count,cylinder_radius)
	cylinder_positions = compute_position(square_count,radius_list)
